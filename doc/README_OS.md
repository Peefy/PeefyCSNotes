...
* **共享内存**-共享内存（Shared Memory），指两个或多个进程共享一个给定的存储区。共享内存是最快的一种 IPC，因为进程是直接对内存进行存取。因为多个进程可以同时操作，所以需要进行同步。**信号量+共享内存**通常结合在一起使用，**信号量**用来同步对共享内存的访问。
...
```
cat filename | tail -n 1000：显示后面1000行
```

**102. Linux下怎么得到文件的第1000行到第3000行**
